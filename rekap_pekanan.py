...
def rekapPekananMasuk(user):
    print(f"{kuning}Rekap Pekanan Transaksi Masuk{default}")
    ...
    print(f"{hijau_tebal}Masukkan pekan yang ingin direkap dari bulan tersebut (1-4){default}")
    ...
    for transaksi in user.transaksi_masuk:
        if (isSameWeek(tanggalRekap, transaksi.waktu)):
    ...
    if (ada_pemasukan==True):
        print(f"{biru_tebal}Total pemasukan pada pekan ke-{tanggalRekap.pekan}, Bulan ", end="")
...
def rekapPekananKeluar(user):
    print(f"{kuning}Rekap Pekanan Transaksi Keluar{default}")
    tanggalRekap = Tanggal(0, 0, 0)

    print(f"{hijau_tebal}Masukkan tahun yang ingin ditampilkan data rekapnya. (Contoh: 2023){default}")
    tanggalRekap.tahun = int(input(">> "))
    print(f"Masukkan bulan yang ingin ditampilkan data rekapnya. (Silakan masukkan angkanya saja){biru_laut}")
    print("1. Januari")
    print("2. Februari")
    print("3. Maret")
    print("4. April")
    ...
    for transaksi in range(len(user.transaksi_keluar)):
        if (isSameWeek(tanggalRekap, transaksi)):
    ...
    if ada_pengeluaran==True:
        print(f"{biru_tebal}Total pengeluaran pada pekan ke-{tanggalRekap.pekan}, bulan ")
        if tanggalRekap.bulan == 1:
            print("Januari: ", end="")
        elif tanggalRekap.bulan == 2:
            print("Februari: ", end="")
...
def tampil_menu_rekap_pekanan(user):
    ...
    print(f"{hijau_tebal}1.{default} Rekap Pekanan Transaksi Masuk")
    print(f"{hijau_tebal}2.{default} Rekap Pekanan Transaksi Keluar")
